...
class params:
    def __init__(self):
        ...
        self.p = int(2**15)  # p 값 m x k 가 p를 넘지 않도록...
        self.L = int(2**10)  # L 값 e x k 가 L을 넘지 않도록... quantization param이 L 의 역할..
        self.r = 10         # 오류 범위
        self.N = 4     # 키 차원 
        # self.p = int(2**70)  # p 값 m x k 가 p를 넘지 않도록...
        # self.L = int(2**4)  # L 값 e x k 가 L을 넘지 않도록... quantization param이 L 의 역할..
        # self.r = 10         # 오류 범위
        # self.N = 1000     # 키 차원 
...
def EncVec_res(m_vec, sk, env):
    ...
    # 마지막에 0으로 채워진 열 추가
    zero_column = np.zeros((n, 1), dtype=object)
    ciphertext = np.hstack((ciphertext, zero_column))  # n x (N+2) 행렬로 변경
